...
public class CharacterLocomotionManager : MonoBehaviour
{
    ...
    protected virtual void HandleGravity()
    {
        ...
        if (character.isGrounded.Value)
        {
            ...
            if (yVelocity.y < 0)
            {
                yVelocity.y = groundYVelocity; // 这里可以设置为0或一个很小的正值
            }
        }
        else
        {
    ...
    {
        character.isGrounded.Value = Physics.CheckSphere( character.transform.position + groundCheckSphereOffset, groundCheckSphereRadius, groundLayer, QueryTriggerInteraction.Ignore);
    }
